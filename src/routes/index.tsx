...
function Hero(){return <section className="home-hero section-light"><HeroField/><div className="wrap hero-inner"><div><h1><span className="mark mark-yellow">AI Automation</span> that learns and adapts to your business.<br/>Deploy <span className="mark mark-coral">AI Agents</span> in your painstaking workflows 👉 GROWTH</h1><p className="hero-copy">We implement most advanced AI models into your existing processes.</p><p className="hero-points"><span>7 Day Builds.</span><span>Fixed Price, not an hourly rate.</span><span>Completely Custom</span></p><div className="hero-actions"><PrimaryLink>TALK TO US</PrimaryLink><Link className="text-link" to="/" hash="solutions">See what we can do. <ArrowRight size={15}/></Link></div></div></div></section>}

function HeroField(){const ref=useRef<HTMLCanvasElement>(null);useEffect(()=>{const c=ref.current;if(!c)return;const ctx=c.getContext("2d");if(!ctx)return;let id=0,t=0;const reduce=matchMedia("(prefers-reduced-motion: reduce)").matches;const draw=()=>{const d=devicePixelRatio||1,w=c.clientWidth,h=c.clientHeight;c.width=w*d;c.height=h*d;ctx.setTransform(d,0,0,d,0,0);ctx.clearRect(0,0,w,h);ctx.font="9px monospace";const chars="@%#*+=-:. ";const gradient=ctx.createLinearGradient(0,0,w,h);gradient.addColorStop(0,"#1e1b4b");gradient.addColorStop(.34,"#0f4c5c");gradient.addColorStop(.67,"#8f1d1d");gradient.addColorStop(1,"#4d5b0a");for(let y=20;y<h;y+=20)for(let x=20;x<w;x+=20){const v=(Math.sin(x*.018+t)+Math.cos(y*.022-t*.8)+2)/4;if(v>.24){ctx.globalAlpha=.14+v*.5;ctx.fillStyle=gradient;ctx.fillText(chars[Math.min(chars.length-1,Math.floor((1-v)*chars.length))]??" ",x,y)}}t+=.025;if(!reduce)id=requestAnimationFrame(draw)};draw();return()=>cancelAnimationFrame(id)},[]);return <canvas ref={ref} className="hero-field" aria-hidden="true"/>}
